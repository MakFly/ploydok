...
function tempSocketPath(): string {
  return path.join(os.tmpdir(), `ploydok-test-${Date.now()}-${Math.random().toString(36).slice(2)}.sock`);
}

/** Démarre un serveur gRPC mock sur un socket unix et retourne { server, socketPath }. */
function startMockServer(impl: Partial<AgentServer>): Promise<{ server: grpc.Server; socketPath: string }> {
  return new Promise((resolve, reject) => {
    ...
    const defaultImpl: AgentServer = {
      containerCreate: (_call, cb) => cb({ code: grpc.status.UNIMPLEMENTED, details: "non implémenté", name: "Error", message: "non implémenté", metadata: new grpc.Metadata() }, null),
      containerStart: (_call, cb) => cb({ code: grpc.status.UNIMPLEMENTED, details: "non implémenté", name: "Error", message: "non implémenté", metadata: new grpc.Metadata() }, null),
      containerStop: (_call, cb) => cb({ code: grpc.status.UNIMPLEMENTED, details: "non implémenté", name: "Error", message: "non implémenté", metadata: new grpc.Metadata() }, null),
      containerRemove: (_call, cb) => cb({ code: grpc.status.UNIMPLEMENTED, details: "non implémenté", name: "Error", message: "non implémenté", metadata: new grpc.Metadata() }, null),
      containerLogs: (call) => { call.end(); },
      containerStats: (call) => { call.end(); },
      imagePull: (call) => { call.end(); },
      ...
      dumpDatabase: (call) => { call.end(); },
      restoreDatabase: (_call, cb) => cb({ code: grpc.status.UNIMPLEMENTED, details: "non implémenté", name: "Error", message: "non implémenté", metadata: new grpc.Metadata() }, null),
    };

    server.addService(AgentService, { ...defaultImpl, ...impl } as grpc.UntypedServiceImplementation);
...
describe("Agent — containerLogs stream", () => {
  ...
  beforeAll(async () => {
    ({ server, socketPath } = await startMockServer({
      containerLogs: (call) => {
        const lines: LogLine[] = [
          { stream: "stdout", line: "ligne 1", timestamp: "2024-01-01T00:00:01Z" },
          { stream: "stdout", line: "ligne 2", timestamp: "2024-01-01T00:00:02Z" },
          { stream: "stderr", line: "ligne 3", timestamp: "2024-01-01T00:00:03Z" },
        ];
