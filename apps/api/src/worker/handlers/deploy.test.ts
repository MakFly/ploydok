...
describe.skipIf(skipIntegration)("handleDeploy — log archiving", () => {
  ...
  it("creates a log file at the expected path and writes log lines to it", async () => {
    ...
    spyOn(detectMod, "detectBuildMethod").mockResolvedValue({ method: "nixpacks" });
    spyOn(registryMod, "diskGuard").mockResolvedValue(undefined);
    spyOn(nixpacksMod, "nixpacksBuild").mockImplementation(async ({ onLog }) => {
    ...
    const { handleDeploy } = await import("./deploy");
    const job = makeJob({ appId: "app-log-1", commitMessage: "chore: log archiving test" });
    ...
    const logFiles = (fs.readdirSync(expectedLogDir) as string[]).filter((f) => f.endsWith(".log"));
  ...
  it("still creates the log file even when the build fails", async () => {
    ...
    await expect(handleDeploy(db, job)).rejects.toThrow("git clone failed: timeout");
...
describe.skipIf(skipIntegration)("handleDeploy — blue-green", () => {
  ...
  it("calls runBlueGreen after push, updates build containerId, publishes Container live event", async () => {
    ...
    spyOn(eventBusMod.eventBus, "publish").mockImplementation((channel: string, event: object) => {
      publishedEvents.push({ channel, event });
    });

    const { handleDeploy } = await import("./deploy");
    const job = makeJob({ appId: LOG_TEST_APP_ID_BG, commitMessage: "feat: test deploy" });

    await handleDeploy(db, job);

    // runBlueGreen must have been called.
    expect(runBlueGreenSpy).toHaveBeenCalledTimes(1);
    const bgOpts = runBlueGreenSpy.mock.calls[0]![0] as import("../runner").RunBlueGreenOptions;
    ...
    const liveEvent = publishedEvents.find(
      (e) =>
        (e.event as { type?: string; message?: string }).type === "deploy.status_change" &&
        (e.event as { message?: string }).message === "Container live",
    );
    expect(liveEvent).toBeDefined();
    expect((liveEvent!.event as { data?: { containerId?: string } }).data?.containerId).toBe("cont-123");
  });
  ...
  it("marks build failed and does NOT set app status to running when runBlueGreen throws", async () => {
    ...
    await expect(handleDeploy(db, job)).rejects.toThrow("healthcheck failed after 6 retries");
    ...
    const appRows = await db.select().from(apps).where(eq(apps.id, LOG_TEST_APP_ID_BG)).limit(1);
