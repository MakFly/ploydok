...
import { apps, databases, app_db_links, secrets, projects } from "@ploydok/db"
...
const LinkBody = z.object({
  env_prefix: z.string().min(1).max(32).regex(/^[A-Z0-9_]+$/).optional().default("DATABASE"),
})
...
export function createAppsDatabasesLinkRouter(db: Db): Hono<any, any, any> {
  ...
  router.post("/:id/databases/:dbId/link", async (c) => {
    ...
    const dbRows = await db
      ...
      .where(and(eq(databases.id, dbId), eq(projects.owner_id, user.id)))
      .limit(1)
    ...
    if (dbRow.project_id !== app.project_id) {
      return c.json(
        { error: { code: "FORBIDDEN", message: "Database and app must belong to the same project" } },
        403,
      )
    }
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(secrets.linked_database_id, dbId),
        ),
      )
    ...
    await db
      ...
      .where(
        and(
          eq(app_db_links.app_id, appId),
          eq(app_db_links.database_id, dbId),
        ),
      )
    ...
    log.info({ appId, dbId, env_prefix, vars: Object.keys(vars) }, "database linked to app")
