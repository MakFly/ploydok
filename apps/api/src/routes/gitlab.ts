...
function verifyState(cookieValue: string, state: string): boolean {
  ...
  const expected = createHmac("sha256", env.SESSION_SECRET).update(state).digest("hex");
...
function buildCookie(name: string, value: string, maxAge: number, httpOnly: boolean): string {
...
gitlabRouter.post("/config", async (c) => {
  ...
  const instanceUrlRaw = (body["instance_url"] as string | undefined) ?? "https://gitlab.com";
  let instanceUrl: URL;
  try {
  ...
  if (instanceUrl.protocol !== "https:" && instanceUrl.hostname !== "localhost" && instanceUrl.hostname !== "127.0.0.1") {
...
gitlabRouter.get("/connect", async (c) => {
  ...
  c.header(
    "Set-Cookie",
    buildCookie(OAUTH_STATE_COOKIE, signState(state), OAUTH_STATE_TTL_SECONDS, true),
  );
...
gitlabRouter.get("/callback", async (c) => {
  ...
  const cookieVal = parseCookie(c.req.header("cookie") ?? "", OAUTH_STATE_COOKIE);
  ...
  if (!tokenRes.ok) {
    ...
    return c.json({ error: "oauth_exchange_failed", status: tokenRes.status }, 502);
  }
  ...
  const rt = tokens.refresh_token ? await encryptField(tokens.refresh_token) : null;
  const expiresAt =
