...
export function createOrganizationsRouter(db: Db): Hono {
  ...
  router.post("/", async (c) => {
    ...
    if (!parsed.success) {
      return c.json(
        { error: { code: "VALIDATION_ERROR", message: "Invalid workspace payload" } },
        400,
      );
    }
  ...
  router.get("/", async (c) => {
    const user = getUser(c);
    const organizations = await listOrganizationsForUser(db, user.id, user.display_name);
    return c.json({ organizations });
  });

  router.get("/default", async (c) => {
    const user = getUser(c);
    const organization = await getDefaultOrganizationForUser(db, user.id, user.display_name);
    return c.json({ organization });
  });

  router.get("/:slug", async (c) => {
    const user = getUser(c);
    const organization = await getOrganizationBySlugForUser(db, user.id, c.req.param("slug"));
    if (!organization) {
