...
export function organizationPath(orgSlug: string, suffix = "dashboard"): string {
  return `/orgs/${orgSlug}/${suffix.replace(/^\/+/, "")}`;
}

export function replaceOrganizationInPath(pathname: string, orgSlug: string): string {
...
export function useOrganizationBySlug(orgSlug: string) {
  return useQuery<OrganizationSummary, ApiError>({
    ...
    queryFn: async () => {
      const data = await apiFetch<OrganizationResponse>(`/organizations/${orgSlug}`);
      return data.organization;
    },
...
export function useCurrentOrganization(): OrganizationSummary | null {
  ...
  const orgMatch = matches.find((match) => match.routeId === "/_authed/orgs/$orgSlug");
  const loaderData = orgMatch?.loaderData as { organization?: OrganizationSummary } | undefined;
