...
describe("apiFetch — auto-refresh sur 401", () => {
  ...
  it("retente après un refresh réussi et renvoie la 2ᵉ réponse", async () => {
    ...
    enqueue(`${BASE}/me`, [
      {
        status: 401,
        body: { error: { code: "UNAUTHENTICATED", message: "expired" } },
      },
  ...
  it("déduplique uniquement les GET simultanés vers le même path", async () => {
  ...
  it("refait un GET réseau une fois la requête précédente résolue", async () => {
    enqueue(`${BASE}/me`, [
      { status: 200, body: { user: { id: "u1" } } },
    ...
    expect(calls.filter((x) => x.url === `${BASE}/me`).length).toBe(2)
  })

  it("retente avec le cookie partagé quand un autre onglet gagne le refresh", async () => {
    enqueue(`${BASE}/me`, [
      { status: 401, body: { error: { code: "UNAUTHENTICATED" } } },
      { status: 200, body: { user: { id: "u1" } } },
    ])
    enqueue(`${BASE}/auth/refresh`, [
      { status: 409, body: { error: { code: "REFRESH_CONFLICT" } } },
    ])
  ...
  it("ne retente pas sur un 401 venant des endpoints pré-session", async () => {
    ...
    await expect(apiFetch("/auth/login/options")).rejects.toBeInstanceOf(
      ApiError
    )
    const urls = calls.map((c) => c.url)
  ...
  it("single-flight: 5 GET concurrents 401 ne déclenchent qu'UN seul refresh", async () => {
    ...
    const refreshCount = calls.filter(
      (c) => c.url === `${BASE}/auth/refresh`
    ).length
  ...
  it("normalise les erreurs reseau directes en BackendUnavailableError", async () => {
    ...
    await expect(apiFetch("/apps")).rejects.toBeInstanceOf(
      BackendUnavailableError
    )
  })
